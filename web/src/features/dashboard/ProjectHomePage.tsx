...
export default function ProjectHomePage() {
  // ACME: Security Analyst and Auditor cannot read the dashboard's trace data
  // (securityRoleAllowList.ts, ADR-0011), so send them to a page they can use.
  const landingRouter = useRouter();
  const landingProjectId = landingRouter.query.projectId as string | undefined;
  const isSecurityAnalyst = useLandsOnGuardrails(landingProjectId);
  useEffect(() => {
    if (isSecurityAnalyst && landingProjectId) {
      landingRouter.replace(
