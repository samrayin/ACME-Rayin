...
| CHG-2026-017 | 2 | Commit the benign-prompt false-positive corpus, its summariser and its runbook — the harness that produced the N-56 measurement — which existed only as untracked files on a single workstation. Documentation and test scaffolding only: no product code, no cluster dependency, nothing deployable. Also records that `gateway-eval.yaml` changes meaning, not behaviour, once the ADR-0005 hook ships | Anees Ur Rahman | `chore/chg-2026-017-promptfoo-benign-corpus` | Claimed |
