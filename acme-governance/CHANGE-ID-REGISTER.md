...
| CHG-2026-029 | 1 | Executes CHG-2026-022/023's prerequisite judge-key change on the live gateway: `cairo-guardrails-judge-rotation-2026-09-20-7fe9a9d4` gets `opted_out_global_guardrails: ["cairo-guardrail"]` in its metadata, `models` narrowed from unrestricted (`[]`) to `["groq-safeguard", "nvidia-nemotron"]`, and `rpm_limit` set to `10`. **All three executed 2026-09-22, verified against LiteLLM's own API response — but not through the designed path.** Executed via the raw LiteLLM `/key/update` API directly, not `updateKeyLimits()` as designed, because no operator-accessible audited path exists (see `ACME-CHANGELOG.md`, same date). Confirmed by direct query: zero `acme_litellm_events` rows for any of the three changes. **New P1 finding raised** (contradicts ADR-0003's single-control-plane premise) and a separate backlog item (an operator-accessible audited path for key mutations). **Not closed** — held pending a drift check on `acme_litellm_keys`; if stale, reconciliation is part of this change, not a follow-up | Anees Ur Rahman | `docs/claim-chg-2026-029-judge-key-hygiene` | Claimed — held, not merged |
