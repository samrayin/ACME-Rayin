...
| CHG-2026-029 | 1 | Executes CHG-2026-022/023's prerequisite judge-key change on the live gateway: `cairo-guardrails-judge-rotation-2026-09-20-7fe9a9d4` gets `opted_out_global_guardrails: ["cairo-guardrail"]` in its metadata, `models` narrowed from unrestricted (`[]`) to `["groq-safeguard", "nvidia-nemotron"]`, and `rpm_limit` set to `10`. Read-merge-write via a one-off script reusing `acmeLitellmClient`'s primitives (not `updateKeyLimits()` directly — extending that shared function for a one-time correction was rejected to avoid touching code other CAIRO features depend on). Full pre-edit key snapshot captured for rollback. **Script prepared, not yet executed — owner-gated live write, held for final confirmation** | Anees Ur Rahman | `docs/claim-chg-2026-029-judge-key-hygiene` | Claimed |
