...
**Last updated:** 2026-09-22 (CHG-2026-030 deployed as `acme-v4.38.0.6`; the judge key was reconciled through it and CHG-2026-029 is closed, confirmed against `acme_litellm_keys`/`acme_litellm_events` directly. Also, Thread 2: **N-51 actually fixed, not just designed** — CHG-2026-034 (PR #94) merged to `main` 2026-09-22T20:30:39Z, `pipeline.yml`'s heavy jobs now run on `ubuntu-latest`, confirmed by a real green-and-red pipeline run, not YAML validity alone. CHG-2026-036 (PR #100) also merged — two more findings from that real run (P1 `layout.clienttest.ts` non-termination bug, still open; `smoke-image.sh` readiness gap, fixed) plus a Codespell false positive. Both of CHG-2026-036's CI-blocking fixes (`smoke-image.sh` retry, `tests-storybook` worker count) shipped inside PR #94, owner-approved after isolated-rerun investigation)
