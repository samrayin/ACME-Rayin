...
**Last updated:** 2026-09-22 (CHG-2026-030 build completed, this session)
...
| Reviewed in one console | 🟡 **Fix built, not yet live** | Console correctly detects drift. CHG-2026-030 (fixes it) is built and gate-passed — typecheck, tests, lint all clean — but not deployed; the console still cannot reconcile drift today, and Rotate still makes it worse until this ships. |
...
| Change | What | Status |
|---|---|---|
| **CHG-2026-030 / ADR-0007** | Wire `updateKeyLimits()` to a Keys page edit dialog — the fix for the console defect above | 🟡 **Built, all gates passed.** Typecheck clean, 6/6 + 28/28 tests pass, lint clean. Source and tests only — **no deploy without explicit owner go-ahead.** |
| **CHG-2026-029** | Judge key drift (models/rpm_limit wrong in CAIRO's own DB; live gateway state is already correct) | 🔴 **Blocked**, waiting on CHG-2026-030 to actually ship, then owner uses the new UI to reconcile CAIRO's record |
| **CHG-2026-031** | This file — adopted as a fourth standing record alongside the changelog, ledger and readiness-auditor prompt | 🟢 Committed |
...
**Next update trigger:** when CHG-2026-030 actually deploys, or when the owner completes the CAIRO-DB reconciliation through it — whichever comes first.
