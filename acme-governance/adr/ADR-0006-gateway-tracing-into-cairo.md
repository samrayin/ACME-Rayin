...
- **Follow-up taken, CHG-2026-054:** both residuals sit in `requester_metadata`, LiteLLM's copy of the caller's metadata and request headers. That object now keeps only `REQUESTER_METADATA_ALLOWLIST` keys, which starts empty. Gateway-written fields (key alias, team, spend) are untouched. **Closed only when the live marker test shows no `GC7Q-HDR` / `GC7Q-RESIDUAL` in ClickHouse or blob.**
- **Closed live, 2026-09-23.** After the CHG-2026-054 hook was redeployed, a fresh marker test (`GC7R-*`) placed markers in every channel, including both `langfuse_*` headers and an arbitrary metadata key. The only marker left in ClickHouse or blob was `GC7R-USER`, from the `user` request field. That is end-user attribution by design, caller-asserted (N-34); keeping it is the owner's decision. The header and free-metadata residuals are closed.
